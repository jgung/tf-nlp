...
def mlp(inputs, inputs_shape, dropout_rate, output_size, training, name, n_splits=1):
    ...
    def _dropout(_inputs):
        ...
        dropped = tf.compat.v1.layers.dropout(_inputs, dropout_rate, noise_shape=[inputs_shape[0], 1, size], training=training)
    ...
    initializer = None
    if training and not tf.compat.v1.get_variable_scope().reuse:
    ...
    result = tf.compat.v1.layers.dense(inputs, output_size, activation=_leaky_relu, kernel_initializer=initializer, name=name)
...
def bilinear(input1, input2, output_size, timesteps, include_bias1=True, include_bias2=True,
    ...
    def _add_bias(_input):
        batches_and_tokens = tf.shape(input=_input)[0]
    ...
    weights = tf.compat.v1.get_variable("weights", shape=[input1dim, output_size, input2dim], initializer=initializer)
    ...
    with tf.compat.v1.variable_scope(name):
        if crf:
            ...
            if label_smoothing > 0:
                ...
                losses = tf.boolean_mask(tensor=losses, mask=mask, name="mask_padding_from_loss")
                n_tokens = tf.cast(tf.reduce_sum(input_tensor=tf.cast(mask, tf.int32)), tf.float32)
                loss = tf.reduce_sum(input_tensor=losses) / n_tokens  # just average over batch/token-specific losses

        if confidence_penalty > 0:
            masked = tf.boolean_mask(tensor=logits, mask=tf.sequence_mask(sequence_lengths))
            # compute entropy, ignoring padding
            entropy = -tf.reduce_sum(input_tensor=tf.nn.log_softmax(masked) * tf.nn.softmax(masked), axis=-1)
            # add the negative entropy to the negative log likelihood
            loss += -tf.reduce_mean(input_tensor=confidence_penalty * entropy)
    return loss
